...
function LogoCard({ brand }: { brand: BrandItem }) {
  const content = (() => {
    if (brand.type === "svg" && brand.logo) {
      ...
      return (
        <SvgLogo className="h-10 w-auto max-w-[130px] text-muted-foreground group-hover:text-foreground transition-all duration-300" />
      );
    }
    if (brand.type === "img" && brand.logoUrl) {
      return (
        <img
          src={brand.logoUrl}
          alt={`${brand.name} logo`}
          className="h-10 max-w-[130px] object-contain opacity-70 group-hover:opacity-100 transition-all duration-300"
          loading="lazy"
          onError={(e) => {
